...
async function main() {
  await Promise.all([
    ...
    esbuild.build({
      ...SHARED_OPTIONS,
      // Fleet MCP stdio bridge -> main-process FleetMcpServer over TCP.
      entryPoints: [path.join(ROOT, 'src/process/services/fleet/fleetMcpStdio.ts')],
      outfile: path.join(ROOT, 'out/main/fleet-mcp-stdio.js'),
    }),
