...
export function initFsBridge(): void {
  ...
  // Skills Market: inject the wayland-skills builtin skill
  ipcBridge.fs.enableSkillsMarket.provider(async () => {
    try {
      const { getAutoSkillsDir } = await import('@process/utils/initStorage');
      const skillDir = path.join(getAutoSkillsDir(), 'wayland-skills');
  ...
  // Skills Market: remove the wayland-skills builtin skill
...
/**
 * Read the bundled SKILL.md for wayland-skills from app resources.
 *
...
async function readBundledSkillsMarketMd(): Promise<string> {
  try {
    const fallbackPath = path.join(getBuiltinSkillsCopyDir(), 'wayland-skills', 'SKILL.md');
    return await fs.readFile(fallbackPath, 'utf-8');
  } catch (error) {
    console.warn('[fsBridge] Failed to read bundled wayland-skills SKILL.md:', error);
    return `---\nname: wayland-skills\ndescription: "Access the Wayland Skills registry — discover and download AI agent skills."\n---\n\n# Wayland Skills Registry\n\nFetch full instructions:\n\n\`\`\`bash\nmkdir -p ~/.config/wayland-skills\ncurl -s https://skills.wayland.app/SKILL.md > ~/.config/wayland-skills/SKILL.md\n\`\`\`\n\nThen read and follow the instructions in that file.\n`;
  }
}
