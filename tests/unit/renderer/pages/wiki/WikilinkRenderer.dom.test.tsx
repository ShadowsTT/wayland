...
describe('WikilinkRenderer', () => {
  it('renders plain text without wikilinks unchanged', () => {
    const { container } = render(<WikilinkRenderer text='Hello world' resolveBacklink={resolver} />);
    expect(container.textContent).toBe('Hello world');
  });

  it('renders a single wikilink as a BacklinkChip', () => {
    render(<WikilinkRenderer text='See [[TurnElement enum]] for details' resolveBacklink={resolver} />);
  ...
  it('renders multiple wikilinks', () => {
    render(
      <WikilinkRenderer text='[[TurnElement enum]] uses [[file-per-backend pattern]]' resolveBacklink={resolver} />
    );
    const chips = screen.getAllByTestId('backlink-chip');
  ...
  it('renders alias display text for [[X|alias]] syntax', () => {
    render(<WikilinkRenderer text='See [[TurnElement enum|the enum]] here' resolveBacklink={resolver} />);
  ...
  it('calls onNavigate when a resolved chip is clicked', () => {
    ...
    render(<WikilinkRenderer text='[[TurnElement enum]]' resolveBacklink={resolver} onNavigate={onNavigate} />);
