...
describe('SiderTeamsSection', () => {
  ...
  it('renders accordion shell with the Teams label', () => {
  ...
  it('hides the badge when no agents are running', () => {
    mockUseTeamList.mockReturnValue({ teams: teamsWithRunning(0) });
    renderSection();
  ...
  it('mounts the preserved SiderActiveTeams inside the Running group when liveCount > 0', () => {
  ...
  it('always mounts the preserved TeamSiderSection inside the My teams group when open', () => {
    mockUseTeamList.mockReturnValue({ teams: teamsWithRunning(0) });
    localStorage.setItem(ACCORDION_STORAGE_KEY, JSON.stringify({ scheduled: false, workflows: false, teams: true }));
    renderSection();
